...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Setup data-lake infrastructures')
    parser.add_argument('--project_id',
                        default='bi-fcom-drmb-loyalty-prd')  # my project
    ...
    # Create datasets
    create_dataset('sandbox_ext_jjaramillo_temp',
                   client)  # my dataset for raw data
    create_dataset('sandbox_ext_jjaramillo',
                   client)  # my dataset for processed data

    # Create staging tables
    for table_id in ss.tables:
        schema = eval('ss.' + table_id)
        create_table(table_id, schema, 'sandbox_ext_jjaramillo_temp',
                     client)  # my dataset for raw data
